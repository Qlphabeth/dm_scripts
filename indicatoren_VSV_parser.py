...
def indicatoren_VSV_parser(filename):
    ...
    The vsv (vroegtijdig school verlaters) are in a dict again, separated in Inst. and Vgl Grp,
    and therein another dict with the years and results in percentages or as nan
    """
    data_dict = {}
    data = pd.read_excel(filename, header=[0, 1, 2], index_col=None, sheet_name='VSV')
...
if __name__ == '__main__':
    data = indicatoren_VSV_parser('01-mbo-indicatoren-per-instelling-2019.xls')
    for row in data:
        print(data[row])
